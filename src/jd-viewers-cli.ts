...
async function cliHandler() {
  if (!process.argv.slice(2).length) {
    ...
    if (cmd === 'vo') {
      ...
      if (command.verbose) console.log('Generating Functional Predictions...');
    ...
    // Save figure to file
    if (command.verbose) console.log(`Generating ${command.outformat.toUpperCase()} output...`);
    let output: any;
...
function makeCommand(cmd: string, description: string, alias: string) {
  ...
  if (cmd === 'fp') command.option('-ix, --inx <xml>', 'URL/URI path to XML (for FP)');
  command
  ...
  else if (cmd === 'fp') command.option('-hits, --numbhits [hits]', 'Number of Hits to display [30]');
  command;
