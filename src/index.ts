...
class CanvasState {
  ...
  addCanvas(jobId: string, data: string, submitter: string) {
    if (this.jobIds.length === 0 || !this.jobIds.includes(jobId) || submitter !== this.submitterName) {
...
class JobIdInputForm {
  ...
  constructor() {
    this.templateElement = document.getElementById('input-jobid')! as HTMLTemplateElement;
    this.hostElement = document.getElementById('jd-viewers-app')! as HTMLDivElement;
    if (this.templateElement !== null) {
      const importedHTMLcontent = document.importNode(this.templateElement.content, true);
      this.element = importedHTMLcontent.firstElementChild as HTMLFormElement;

      this.jobIdElement = this.element.querySelector('#jobid')! as HTMLInputElement;
  ...
  private submitHandler(event: Event) {
    ...
    if (validateJobId(formValidatable)) {
      if (jobId === 'mock_jobid-I20200317-103136-0485-5599422-np2') {
        canvasInstance.addCanvas(jobId, 'testdata/ncbiblast.json', submittedEvent.submitter.name.trim());
      } else {
        canvasInstance.addCanvas(jobId, jobId, submittedEvent.submitter.name.trim());
        return;
      }
    } else {
...
class FabricjsRenderer {
  ...
  constructor() {
    this.templateElement = document.getElementById('jd-viewers-output')! as HTMLTemplateElement;
    this.hostElement = document.getElementById('jd-viewers-app')! as HTMLDivElement;
    if (this.templateElement !== null) {
      const importedHTMLcontent = document.importNode(this.templateElement.content, true);
  ...
  private async renderCanvas() {
    ...
    if (this.canvasInstance.submitter === 'visual-output') {
      ...
      let iprmcXmlData;
      if (this.canvasInstance.jobId === 'mock_jobid-I20200317-103136-0485-5599422-np2') {
    ...
    document.getElementById('btn-svg')!.onclick = function () {
      const img: HTMLImageElement = document.getElementById('svg') as HTMLImageElement;
